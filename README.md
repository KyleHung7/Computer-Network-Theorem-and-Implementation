...
| 項目           | 請求（Request）                           | 回應（Response）                                |
|----------------|------------------------------------------|-----------------------------------------------|
| **時間戳**     | 1.413853 秒                              | 1.598800 秒                                   |
| **來源 IP**    | 192.168.102.232                          | 34.223.124.45 (伺服器)                        |
| **目標 IP**    | 34.223.124.45 (伺服器)                   | 192.168.102.232 (客戶端)                      |
| **協議**       | HTTP                                     | HTTP                                          |
| **封包大小**   | 211 bytes                                | 1439 bytes                                    |
| **HTTP 請求**  | `GET / HTTP/1.1`                         | `HTTP/1.1 200 OK (text/html)`                 |
| **描述**       | 客戶端發送請求根目錄 (`/`) 的 HTML 頁面。  | 伺服器返回 200 OK，並返回 HTML 內容。         |

### 解析：
- **請求**：客戶端發送了 `GET` 請求，請求的是根目錄 (`/`) 的 HTML 頁面。
- **回應**：伺服器返回了狀態碼 `200 OK`，表示請求成功，並返回了 HTML 內容。
...
| 項目           | 請求（Request）                           | 回應（Response）                                |
|----------------|------------------------------------------|-----------------------------------------------|
| **時間戳**     | 1.920501 秒                              | 2.163328 秒                                   |
| **來源 IP**    | 192.168.102.232                          | 52.54.161.49 (伺服器)                         |
| **目標 IP**    | 52.54.161.49 (伺服器)                    | 192.168.102.232 (客戶端)                      |
| **協議**       | HTTP                                     | HTTP                                          |
| **封包大小**   | 210 bytes                                | 887 bytes                                     |
| **HTTP 請求**  | `GET / HTTP/1.1`                         | `HTTP/1.1 200 OK (text/html)`                 |
| **描述**       | 客戶端發送請求根目錄 (`/`) 的 HTML 頁面。  | 伺服器返回 200 OK，並返回 HTML 內容。         |
